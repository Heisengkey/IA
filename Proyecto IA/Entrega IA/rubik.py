...
estadoS_Yperm = ["yan", "yvn", "yvr", "yar",
                "bvr", "bvn", "ban", "bar"]

estadoS_Tperm = ["ynv", "yrv", "yan", "yar",
                "bvr", "bvn", "ban", "bar"]
...
def perm_Y(estado):
    return [estado[2], estado[1], estado[0], estado[3], estado[4], estado[5], estado[6], estado[7]]

def perm_T(estado):
    return [rotarPegatinaU(estado[1]), rotarPegatinaU(estado[0]), estado[2], estado[3], estado[4], estado[5], estado[6], estado[7]]

def scramble_cube(cube, secuencia):
    for mov in secuencia:
        cube.rota(mov)
    return cube

def procesaSecuencia(secuencia):
    procesada = ''

    for i in range(len(secuencia)):
        if i<=len(secuencia)-4 and secuencia[i] == secuencia[i + 1] and secuencia[i + 1] == secuencia[i + 2] and secuencia[i + 2] == secuencia[i + 3]:
            procesada = procesada
            i += 3
        elif i<=len(secuencia)-3 and secuencia[i] == secuencia[i + 1] and secuencia[i + 1] == secuencia[i + 2]:
            procesada += f"{secuencia[i]}' "
            i += 2
        elif i<=len(secuencia)-2 and secuencia[i] == secuencia[i + 1]:
            procesada += f"2{secuencia[i]}"
            i += 1
...
# print (diferenciaSecuencia(['bvn', 'bvr', 'nab', 'nay', 'yvn', 'yvr', 'rab', 'ray'], ['nvy', 'nvb', 'nab', 'nay', 'rvy', 'rvb', 'rab', 'ray']))
print(diferenciaSecuencia(solved_Yellow_Up, estadoS_Yperm))
print(diferenciaSecuencia(solved_Yellow_Up, estadoS_Tperm))